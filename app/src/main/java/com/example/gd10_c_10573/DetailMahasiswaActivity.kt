...
class DetailMahasiswaActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding =
            ActivityDetailMahasiswaBinding.inflate(layoutInflater)setContentView(binding.root)
        b = intent.extras
        ...
        RClient.instances.getData(nim).enqueue(object : Callback<ResponseDataMahasiswa>{
            override fun onResponse(
                ...
                if(response.isSuccessful){
                    response.body()?.let {
                        listMahasiswa.addAll(it.data) }with(binding) {
    ...
    fun deleteData(nim:String){
        val builder =
            AlertDialog.Builder(this@DetailMahasiswaActivity)builder.setMessage("Anda Yakin mau hapus?? Saya ngga yakin loh.")
                .setCancelable(false)
...
private fun doDeleteData(nim:String) {
    RClient.instances.deleteData(nim).enqueue(object :Callback<ResponseCreate>{
        override fun onResponse(
            ...
            if (response.isSuccessful) {
                Toast.makeText(applicationContext, "Data berhasil dihapus", Toast.LENGTH_LONG).show()finish()
            }
        }
